    ...
    <form onSubmit={handleSubmit} className="space-y-5">
      ...
      <div>
        <div className="flex items-center justify-between mb-1.5">
          <label
            htmlFor="password"
            className="block text-xs uppercase tracking-wider text-sand-50/60 font-medium"
          >
            Password
          </label>
          <Link
            href="/forgot-password"
            className="text-xs text-turq-400 hover:text-turq-300 transition-colors font-medium"
          >
            Forgot password?
          </Link>
        </div>
      ...
      <p className="text-center text-xs text-sand-50/50 pt-2">
        ...
        <Link
          href={`/signup${redirectUrl !== "/dashboard" ? `?redirect_url=${encodeURIComponent(redirectUrl)}` : ""}`}
