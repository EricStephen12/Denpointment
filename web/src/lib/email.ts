  ...
  await sendEmail(
    ...
    `
      <div style="font-family: sans-serif; max-width: 480px; margin: 0 auto;">
        ...
        <p>Hi ${patientName},</p>
        <p>Your appointment with <strong>Dr. ${dentistName}</strong> at ${CLINIC_NAME} has been booked:</p>
        ...
        <p style="color: #94a3b8; font-size: 12px; margin-top: 32px;">${CLINIC_NAME}</p>
      </div>
    `
  );
}
...
export async function sendAppointmentReminderEmail(params: {
  ...
  await sendEmail(
    to,
    `Reminder: Your appointment is coming up — ${CLINIC_NAME}`,
    `
      <div style="font-family: sans-serif; max-width: 480px; margin: 0 auto;">
...
export async function sendPaymentReceiptEmail(params: {
  ...
  await sendEmail(
    to,
    `Payment Received — ${CLINIC_NAME}`,
    `
      <div style="font-family: sans-serif; max-width: 480px; margin: 0 auto;">
        <h2 style="color: #0f172a;">Payment Received</h2>
        <p>Hi ${patientName},</p>
        <p>We've received your payment of <strong>$${amount}</strong> for <strong>${serviceName}</strong>. Thank you!</p>
        <p style="color: #94a3b8; font-size: 12px; margin-top: 32px;">${CLINIC_NAME}</p>
      </div>
    `
  );
